...
def plot_histogram(d: int, num_bins=20) -> None:
    with open(f"results/bike_size_{d}.txt", "rt") as count_file:
        numbers = [int(count) for count in count_file]

    #title = f"Citi Bike, window = {d} seconds"
    fig, axs = plt.subplots(1, 1, figsize=(6, 0.75*4))
    ax = axs
    #ax.set_title(title)
    ax.set_ylabel("frequency")
    ax.set_xlabel("# of records in window")
    ax.semilogy()
    ...
    fig.savefig(
        "figures/" + "citi"+ "_winsize" + f"{d}" + ".pdf", bbox_inches="tight"
    )
    plt.close(fig)
