...
def read_throughput(preamble, agg, functions):
    with open('results/' + preamble + '_' + agg + '.csv', 'r') as file:
        ...
        for row in reader:
            f = row[0]
            w = int(row[1])
            data[f][w] = Data(agg, w, -1, # no degree
...
def read_mfgdebs_throughput(agg, functions):
    with open('results/mfgdebs_data' + '_' + agg + '.csv', 'r') as file:
        ...
        for row in reader:
            ...
            remainder = zip(*[iter(row[2:])]*2) # tuples of (num items processed, execution time)
            throughputs = [int(num)/float(time) for num, time in remainder]
            data[f][w] = Data(agg, w, -1, # no degree
                              np.average(throughputs),
                              np.std(throughputs))
    return data
...
def read_shared_throughput_data(preamble, agg, functions):
    with open('results/' + preamble + '_' + agg + '.csv', 'r') as file:
        ...
        for row in reader:
            ...
            bw = int(row[1])
            sw = int(row[2])
            data[f][bw][sw] = SharedData(agg, bw, sw, 
                                         np.average([(functions[f])/float(x) for x in row[2:]]),
                                         np.std([(functions[f])/float(x) for x in row[2:]]))
    return data
...
def make_throughput_graph(title, preamble, varying, function, aggs, aggregators):
    ...
    for aggregator in aggregators:
        ...
        ax.errorbar(x_axis, throughput, yerr=stddev, 
                    label=get_screen_name(aggregator.name), linewidth=2, linestyle=aggregator.style, color=aggregator.color)

    # shrink axis by 25% and put legend outside and to the right
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 1.1, box.height * 0.75])
    ax.legend(frameon=False, ncol=3, loc='upper center', columnspacing=0.5, bbox_to_anchor=(0.5, 1.55))

    graph.savefig('figures/' + preamble + '_' + function + '.pdf')

def make_mfgdebs_throughput_graph(title, preamble, varying, function, aggs, aggregators):
    graph = plt.figure()
    ax = graph.add_subplot(111)
    ax.set_title(title + ' ' + function)
    ax.set_xlabel(varying)
    ...
    for aggregator in aggregators:
        data = aggs[aggregator.name]
        x_axis = np.array(sorted([float(w) for w in data.keys()]))
    ...
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 1.1, box.height * 0.75])
    ax.legend(frameon=False, ncol=3, loc='upper center', columnspacing=0.5, bbox_to_anchor=(0.45, 1.55))

    graph.savefig('figures/' + preamble + '_' + function + '.pdf')

def make_small_window_varying_graphs(title, preamble, varying, aggregators, func_to_data):
...
def make_mfgdebs_small_window_varying_graphs(title, varying, aggregators, func_to_data):
    for f, aggs in func_to_data.items():
        make_mfgdebs_throughput_graph(title, 'mfgdebs', varying, f, aggs, aggregators)
