#ifndef __STLDequeBased_H__
#define __STLDequeBased_H__
...
namespace daba {
  ...
  template <bool caching, class BinaryFunction, class T>
  Aggregate<BinaryFunction, caching> make_aggregate(BinaryFunction f, T elem) {
    return Aggregate<BinaryFunction, caching>(f, elem);
  }

  template <typename BinaryFunction, bool caching>
  struct MakeAggregate {
    template <typename T>
    Aggregate<BinaryFunction, caching> operator()(T elem) {
      BinaryFunction f;
      return make_aggregate<caching>(f, elem);
    }
  };
}
