...
int main(int argc, char** argv) {
    ...
    if (!(query_call_ooo_benchmark<btree::MakeAggregate, timestamp, 2, btree::finger>("bfinger2", aggregator, function, exp) ||
          query_call_ooo_benchmark<btree::MakeAggregate, timestamp, 4, btree::finger>("bfinger4", aggregator, function, exp) ||
          query_call_ooo_benchmark<btree::MakeAggregate, timestamp, 8, btree::finger>("bfinger8", aggregator, function, exp) ||
    ...
    if (latency) {
        std::ofstream out("results/latency_ooo_" + aggregator + "_" + function + "_w" + window_size_str + "_d" + degree_str + ".txt");
